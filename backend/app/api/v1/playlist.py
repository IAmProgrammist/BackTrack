...
from app.schemas.playlist import PlaylistDetailedResponse, PlaylistEmptyResponse, PlaylistFilter, PlaylistInCreate, PlaylistListResponse, PlaylistPagination, PlaylistResponse, PlaylistSort
...
@router.get("/{playlist_id}", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_playlist", response_model=PlaylistDetailedResponse)
...
@router.get("", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_playlists", response_model=PlaylistListResponse)
async def get_playlists(
    *,
    ...
    response = await playlist_service.get_playlists(playlists_filters=playlists_filters, playlists_pagination=playlists_pagination, playlists_sort=playlists_sort, playlist_repo=playlist_repo)

    return await handle_result(response)


@router.post("", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="create_playlist", response_model=PlaylistResponse)
async def create_playlist(
    *,
    ...
    response = await playlist_service.create_playlist(
        playlist_in=playlist_in, playlist_repo=playlist_repo, songs_repo=songs_repo, files_service=files_service, file_repository=file_repository, settings=settings, token_user=token_user
    )

    return await handle_result(response)


@router.put("/{playlist_id}", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="update_playlist", response_model=PlaylistResponse)
...
@router.delete("/{playlist_id}", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="delete_playlist", response_model=PlaylistEmptyResponse)
async def delete_playlist(
    *,
    ...
    response = await playlist_service.delete_playlist(playlist_repo=playlist_repo, token_user=token_user, playlist_id=playlist_id)
