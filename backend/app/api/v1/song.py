...
@router.get("/{song_id}", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_song", response_model=SongDetailedResponse)
async def get_song(
    *,
    ...
    song = await song_service.get_song_releases_by_id_and_release_id(song_repo=song_repo, song_id=song_id, release_id=release_id, playlist_repo=playlist_repo)

    return await handle_result(song)


@router.get("", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_songs", response_model=SongListResponse)
async def get_songs(
    *,
    song_repo: SongsRepository = Depends(get_repository(SongsRepository)),
...
@router.get("/{song_id}/releases", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_song_releases", response_model=SongReleaseResponse)
async def get_songs_releases(
    *,
    ...
    songs = await song_service.get_song_releases_minified(song_repo=song_repo, filter_=filter_, pagination=pagination, sort=sort, song_id=song_id)

    return await handle_result(songs)


@router.post("", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="create_song", response_model=SongShortResponse)
...
@router.delete("/{song_id}", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="delete_song", response_model=SongEmptyResponse)
async def delete_song(
    *,
    ...
    response = await song_service.delete_song(song_repo=song_repo, song_id=song_id, token_user=token_user)

    return await handle_result(response)


@router.get("/{song_id}/comments", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="get_song_comments", response_model=SongCommentListResponse)
async def get_song_comments(
    *,
    ...
    comments = await song_service.get_comments(comment_repo=comment_repo, song_repo=song_repo, filter_=filter_, sort=sort, pagination=pagination, song_id=song_id)

    return await handle_result(comments)


@router.post("/{song_id}/comments", status_code=HTTP_200_OK, responses=ERROR_RESPONSES, name="create_song_comment", response_model=SongCommentResponse)
async def create_song_comment(
    *,
    ...
    comment = await song_service.create_comment(comment_in=comment_in, comment_repo=comment_repo, song_repo=song_repo, token_user=token_user, song_id=song_id)
