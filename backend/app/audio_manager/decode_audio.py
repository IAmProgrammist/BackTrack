...
def collect_slices(input_path: Path) -> tuple[bytes, dict]:
    ...
    if not bin_files:
        raise FileNotFoundError(f"No slice files (slice_*.bin) found in '{input_path}'.")

    logger.info(f"Found {len(bin_files)} slice file(s) in '{input_path}'.")
...
def write_wav(output_path: str, amplitude_bytes: bytes, metadata: dict):
    ...
    logger.info(f"WAV file written: {output_path}")
    logger.info(f"  Channels    : {channels} (mono)")
    logger.info(f"  Sample rate : {sample_rate} Hz")
    logger.info(f"  Sample width: {sample_width} byte(s) (16-bit PCM)")
...
def main():
    parser = argparse.ArgumentParser(description="Decode binary amplitude slice(s) back to a WAV file.")
    parser.add_argument("--input", "-i", required=True, help="Path to a single .bin slice file OR a directory of slice files")
    parser.add_argument("--output", "-o", required=True, help="Path for the output WAV file (e.g. restored.wav)")
