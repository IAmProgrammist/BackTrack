...
class AutoencoderLoss(nn.Module):
    ...
    def _multiscale_stft_loss(self, x_hat, x):
        ...
        for fft_size in [512, 1024, 2048]:
            ...
            loss += torch.mean(torch.abs(torch.log(orig_mag + 1e-7) - torch.log(rec_mag + 1e-7)))
    ...
    def _perceptual_loss(self, x_hat, x):
        ...
        f2 = freqs**2
        aw = (12200**2 * f2**2) / ((f2 + 20.6**2) * torch.sqrt((f2 + 107.7**2) * (f2 + 737.9**2)) * (f2 + 12200**2))
...
class BinFileDataset(Dataset):
    def __init__(self, root_dir: str):
        ...
        if skipped:
            logger.info(f"  [!] Skipped {skipped} file(s) with wrong size " f"(expected {FILE_BYTES} bytes = {INPUT_DIM} bits each)")
        self.files = valid
        if not self.files:
            raise ValueError(f"No valid .bin files of {FILE_BYTES} bytes found under: {root_dir}")
        logger.info(f"  Found {len(self.files)} valid .bin file(s) in '{root_dir}'")
    ...
    def __getitem__(self, idx):
        ...
        bits = np.array(np.frombuffer(raw, dtype=np.int16)).astype(np.float32) / 32768.0
...
def load_bin_file(path: str) -> torch.Tensor:
    ...
    if p.stat().st_size != FILE_BYTES:
        raise ValueError(f"Expected {FILE_BYTES} bytes ({INPUT_DIM} bits), " f"got {p.stat().st_size} bytes: {path}")
...
def reconstruction_quality(x_hat: torch.Tensor, x: torch.Tensor):
...
def run_train(args):
    ...
    logger.info(f"  Out  : {args.model_out}")
    logger.info(f"  Epochs / batch / lr : {args.epochs} / {args.batch_size} / {args.lr}")
    ...
    if not args.model:
        logger.info("No checkpoint provided")
        model = AudioAutoencoder()
        model.to(device)
    else:
        logger.info("A checkpoint is provided")
        model = load_model(args.model, device)

    total_p = sum(p.numel() for p in model.parameters())
    logger.info(f"  Model params     : {total_p:,}")
    logger.info(f"  Input dim        : {INPUT_DIM:,} nums")
    logger.info(f"  Latent dim       : {LATENT_DIM:,} float16 values")
    logger.info(f"  Value ratio      : {INPUT_DIM / LATENT_DIM:.0f}×")
    logger.info(f"  Bit ratio        : {INPUT_DIM / (LATENT_DIM * 16):.2f}×\n")

    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=10, factor=0.5)
    criterion = MSEAutoencoderLoss(latent_weight=1e-4)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_model(model, args.model_out, meta={"epoch": epoch, "best_loss": best_loss, "history": history})
            tag = "  ← best"
    ...
    logger.info(f"  Saved: {args.model_out}\n")
...
def run_predict(args):
    ...
    logger.info(f"  OutDir: {args.out_dir}")
    logger.info(f"  Device: {device}")
    logger.info(f"{'='*60}\n")
    ...
    with torch.no_grad():
        ...
        logger.info(f"  Compressed  → {latent_path}")
        logger.info(f"    Original : {FILE_BYTES:,} bytes  ({INPUT_DIM:,} bits)")
        logger.info(f"    Latent   : {latent_bytes:,} bytes  ({LATENT_DIM} × float16)")
        logger.info(f"    Ratio    : {FILE_BYTES/latent_bytes:.2f}× (bytes)  |  " f"{INPUT_DIM/(LATENT_DIM*16):.3f}× (bits)\n")

        # ── Decompress ───────────────────────────────────────
        x_hat_soft = model.decode(z)  # (1, 262144)
        x_hat_bin = (x_hat_soft.clamp(-1, 1).cpu() * 32768.0).to(torch.int16)
        logger.info(len(x_hat_bin))
        logger.info(x_hat_bin)
        recon_path = out / f"{stem}_reconstructed.bin"
        save_bin_file(x_hat_bin, str(recon_path))
        logger.info(f"  Decompressed → {recon_path}")

        # ── Quality ───────────────────────────────────────────
        acc = x_hat_bin.eq(x.squeeze(0).cpu()).float().mean().item()
        errors = int((1.0 - acc) * INPUT_DIM)
        logger.info(f"\n  Bit accuracy : {acc*100:.4f}%")
        logger.info(f"  Bit errors   : {errors:,} / {INPUT_DIM:,}")
...
def run_read(args):
    ...
    logger.info(json.dumps(checkpoint))
...
def build_parser():
    ...
    tr.add_argument("--data_dir", required=True, help="Root directory (scanned recursively for .bin files)")
    tr.add_argument("--model_out", required=True, help="Where to save the best model checkpoint (.pt)")
    tr.add_argument("--epochs", type=int, default=10)
    tr.add_argument("--batch_size", type=int, default=8)
    tr.add_argument("--lr", type=float, default=1e-4)
    tr.add_argument("--model", required=False, help="Path to a saved .pt checkpoint")

    # predict
    pr = sub.add_parser("predict", help="Compress + decompress a single .bin file")
    pr.add_argument("--model", required=True, help="Path to a saved .pt checkpoint")
    pr.add_argument("--input", required=True, help="Input .bin file (must be exactly 32768 bytes)")
    pr.add_argument("--out_dir", required=True, help="Directory for output .latent and reconstructed .bin")

    rr = sub.add_parser("read", help="Read data of a model")
    rr.add_argument("--model", required=True, help="Path to a saved .pt checkpoint")
