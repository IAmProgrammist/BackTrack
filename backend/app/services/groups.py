...
class GroupsService(BaseService):
    ...
    async def get_groups(
        ...
        groups = await group_repo.get_groups_with_participants(filter_=groups_filters, pagination=groups_pagination, sort=groups_sort)
        ...
        return dict(
            ...
            content={
                ...
                "data": jsonable_encoder(
                    [GroupOutData(id=group.id, name=group.name, description=group.description, file_id=group.file_id, authors=[author.name for author in group.authors]) for group in groups]
                ),
            },
        )
    ...
    async def create_group(
        ...
        stored_file = await files_service.create_file(file_repository=file_repository, settings=settings, file=group_in.file)
        if not stored_file:
        ...
        group = await group_repo.create_group(group_in=GroupInDB(name=group_in.name, description=group_in.description, file_id=stored_file.id), authors=authors)
        if not group:
        ...
        return dict(
            ...
            content={
                "message": constant.SUCCESS_GROUP_FOUND,
                "data": jsonable_encoder(GroupOutData(id=group.id, name=group.name, description=group.description, file_id=group.file_id, authors=[author.name for author in authors])),
            },
        )
    ...
    @return_service
    async def delete_group(self, group_repo: GroupsRepository = Depends(get_repository(GroupsRepository)), token_user: User = None, group_id: UUID = None) -> GroupEmptyResponse:
