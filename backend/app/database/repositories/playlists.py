...
from app.schemas.playlist import PlaylistFilter, PlaylistInDB, PlaylistPagination, PlaylistSort
...
class PlaylistsRepository(BaseRepository):
    ...
    async def get_playlist_by_id_minified(self, *, playlist_id: UUID) -> Playlist:
        playlist = (await self.connection.execute(select(Playlist).join(Playlist.songs, isouter=True).filter(Playlist.id == playlist_id).limit(1))).scalar()

        return playlist

    @db_error_handler
    async def get_playlists(self, *, filter_: PlaylistFilter, pagination: PlaylistPagination, sort: PlaylistSort) -> list[Playlist]:
        query = select(Playlist).distinct(Playlist.id).join(Playlist.songs, isouter=True).order_by(Playlist.id)

        query = append_to_statement(statement=query, model=Playlist, filter_=filter_, pagination=pagination, sort=sort)
    ...
    @db_error_handler
    async def create_playlist(self, *, playlist_in: PlaylistInDB, file: File) -> Playlist:
    ...
    @db_error_handler
    async def attach_songs_to_playlist(self, *, playlist: Playlist, songs: list[Song], filters: list[str]) -> list[PlaylistSong]:
        ...
        filter_songs = [PlaylistSong(playlist=playlist, song=song, filter=filter) for (song, filter) in zip(songs, filters)]
    ...
    @db_error_handler
    async def update_playlist(self, *, playlist: Playlist, playlist_in: PlaylistInDB, file: File) -> Playlist:
    ...
    @db_error_handler
    async def get_playlists_for_song_release(self, *, song_release: SongRelease) -> list[Playlist]:
        playlists = (
            (
                await self.connection.execute(
                    select(Playlist).join(Playlist.songs, isouter=True).join(PlaylistSong.song).join(Song.song_releases, isouter=True).filter(SongRelease.id == song_release.id)
                )
            )
            .scalars()
            .all()
