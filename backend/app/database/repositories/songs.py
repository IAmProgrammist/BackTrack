...
from app.schemas.song import SongFilter, SongPagination, SongReleaseInDB, SongSort
...
class SongsRepository(BaseRepository):
    ...
    @db_error_handler
    async def get_song_releases(self, *, filter_: SongFilter, pagination: SongPagination, sort: SongSort) -> list[Song]:
        ...
        query = append_to_statement(statement=query, model=SongRelease, filter_=SongFilter(**filter_.model_dump(exclude_unset=True, exclude_none=True)), pagination=pagination, sort=sort)
    ...
    @db_error_handler
    async def get_song(self, *, song_id: UUID) -> Song:
        song = (await self.connection.execute(select(Song).filter(Song.id == song_id).limit(1))).scalar()
    ...
    @db_error_handler
    async def create_song_release(self, *, song_in: SongReleaseInDB, song: Song, authors: list[Author], groups: list[Group]) -> SongRelease:
    ...
    @db_error_handler
    async def attach_files_to_song_release(self, *, song_release: SongRelease, files: list[File], leading: list[bool]) -> list[SongReleaseFile]:
    ...
    @db_error_handler
    async def get_songs_with_ids(self, *, ids: list[UUID]) -> list[Song]:
