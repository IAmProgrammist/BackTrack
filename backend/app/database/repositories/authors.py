...
from app.schemas.author import AuthorFilter, AuthorInDB, AuthorPagination, AuthorSort
...
class AuthorsRepository(BaseRepository):
    ...
    @db_error_handler
    async def get_authors(self, *, filter_: AuthorFilter, pagination: AuthorPagination, sort: AuthorSort) -> list[Author]:
        query = append_to_statement(statement=select(Author), model=Author, filter_=filter_, pagination=pagination, sort=sort)
    ...
    @db_error_handler
    async def get_authors_with_ids(self, *, ids: list[UUID]) -> list[Author]:
        authors = (await self.connection.execute(select(Author).filter(Author.id.in_(ids)))).scalars().all()

        return authors

    @db_error_handler
    async def create_author(self, *, author_in: AuthorInDB) -> Author:
    ...
    @db_error_handler
    async def update_author(self, *, author: Author, author_in: AuthorInDB) -> Author:
